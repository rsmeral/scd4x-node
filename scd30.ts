 ...
 */
export const setAutomaticSelfCalibration = async (enable: boolean): Promise<void> =>
  performCommand(bus, SCD30_CMD_AUTO_SELF_CALIBRATION, booleanToUint16(enable));

/**
 * Indicates whether automatic self-calibration is active.
 */
export const isAutomaticSelfCalibrationActive = async (): Promise<boolean> => {
  const response = await performCommandAndRead(bus, SCD30_CMD_AUTO_SELF_CALIBRATION, 3);
 ...
 * CO2 calibration curve which persists after repowering the sensor. The most recently used reference value is retained
 * in volatile memory and can be read out with `getForcedRecalibrationValue`.
 *
 ...
 */
export const getForcedRecalibrationValue = async (): Promise<number> => {
  const response = await performCommandAndRead(bus, SCD30_CMD_SET_FORCED_RECALIBRATION, 3);
 ...
 */
export const setTemperatureOffset = async (offset: number): Promise<void> =>
  performCommand(bus, SCD30_CMD_SET_TEMPERATURE_OFFSET, integerToUint16(offset));

/**
 * Returns the temperature offset.
 */
export const getTemperatureOffset = async (): Promise<number> => {
  const response = await performCommandAndRead(bus, SCD30_CMD_SET_TEMPERATURE_OFFSET, 3);
 ...
 */
export const setAltitudeCompensation = async (altitude: number): Promise<void> =>
  performCommand(bus, SCD30_CMD_SET_ALTITUDE, integerToUint16(altitude));

/**
 * Returns the altitude compensation value in meters.
 */
export const getAltitudeCompensation = async (): Promise<number> => {
  const response = await performCommandAndRead(bus, SCD30_CMD_SET_ALTITUDE, 3);
...
export const getFirmwareVersion = async (): Promise<string> => {
  const response = await performCommandAndRead(bus, SCD30_CMD_READ_FIRMWARE_VERSION, 3);
 ...
 * will reload all calibrated data. However, it is worth noting that the sensor reloads calibration data prior to every
 * measurement by default. This includes previously set reference values from ASC or FRC as well as temperature offset
 * values last setting.
 *
 * The sensor is able to receive the command at any time, regardless of its internal state.
 */
export const softReset = (): Promise<void> => performCommand(bus, SCD30_CMD_SOFT_RESET);
